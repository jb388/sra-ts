...
(ref:plot-SOC-profiles-cap) Profiles of soil C stocks. Points show mean of 2001, 2009, and 2019 data; error bars show ±2SE.

```{r plot-soc-stocks, fig.cap="(ref:plot-SOC-profiles-cap)", fig.width=4, fig.height=4.05, out.height="65%", out.width="65%"}
# 2001
sra.01.soc.df <- Reduce(
  merge,
  lapply(4:6, function(i) {
    df <- sra.01.blkC %>%
      select(pro_name, lyr_top, lyr_bot, C., bd.g.cm3, fine.earth.) %>%
      select(1:3, i) %>% 
      data.frame
    bind_rows(
      lapply(split(df, df$pro_name), function(x) {
        var_name <- names(x)[4]
        suppressMessages(
          Cspline.fx(x, var.name = var_name, show.progress = FALSE)) %>%
          mutate(lyr_bot = 
                   as.numeric(
                     unlist(
                       lapply(strsplit(depth, "-"), "[[", 2))),
                 lyr_top = as.numeric(
                     unlist(
                       lapply(strsplit(depth, "-"), "[[", 1))))
      }), .id = "pro_name")
  })
) %>%
  mutate(soc_kgm2 = 
           (lyr_bot - lyr_top) * C. * bd.g.cm3 * fine.earth. * 10^-3,
         PMeco = substr(pro_name, 1, 4),
         year = 2001)

# 2009
sra.09.soc.df <- Reduce(
  merge,
  lapply(4:6, function(i) {
    df <- sra.09.blkC %>%
      select(pro_name, top.mineral, bottom.mineral,  
             C_pct, BD_g_cm_3, Soil_finefraction) %>% 
      rename(lyr_top = top.mineral, lyr_bot = bottom.mineral) %>%
      select(1:3, i) %>% 
      data.frame
    bind_rows(
      lapply(split(df, df$pro_name), function(x) {
        var_name <- names(x)[4]
        suppressMessages(Cspline.fx(x, var.name = var_name, show.progress = FALSE)) %>%
          mutate(lyr_bot = 
                   as.numeric(
                     unlist(
                       lapply(strsplit(depth, "-"), "[[", 2))),
  ...
  mutate(soc_kgm2 = C_pct * BD_g_cm_3 * Soil_finefraction,
         PMeco = substr(pro_name, 1, 4),
         year = 2009)

# Combine
sra.19.soc.reps.30 <- merge(
  sra.19.blkC.reps.30,
  sra.09.soc.df[ , c("pro_name", "depth", "BD_g_cm_3", "Soil_finefraction")], 
  by = c("depth", "pro_name")) %>%
  mutate(soc_kgm2 = c_pct * BD_g_cm_3 * Soil_finefraction)
nms.soc <- c("pro_name", "PMeco", "depth", "soc_kgm2")
sra.19.09.01.soc.df <- rbind(
  sra.19.soc.reps.30[ , nms.soc] %>% mutate(year = 2019),
  sra.09.soc.df[ , nms.soc] %>% mutate(year = 2009),
  sra.01.soc.df[ , nms.soc] %>% mutate(year = 2001)) %>%
  group_by(PMeco, depth) %>%
  add_tally() %>%
