...
# 2001 data
sra.2001 <- vector(mode = "list", length = length(unique(sra.2019$pro_name)))
names(sra.2001) <- unique(sra.2019$pro_name)
...
Radiocarbon analyses for the 2001 samples were not run originally, but were completed on archived samples in 2020. All profiles for each site are plotted below in order to give a sense of the within site variability.

```{r merge-iso-data}
# Extract 14C data for 2001 samples
for(i in seq_along(sra.2001)) {
  sra.2001[[i]] <- lapply(ams_results_ls, function(ls) {
...
pro.plot <- function(df) {
  ggplot(df, aes(d14c, lyr_bot, color = PM, shape = ECO, group = pro_rep)) +
    ...
    scale_color_manual(values = c("AN" = "blue", 
                                  "BS" = "red", 
                                  "GR" = "darkgray")) +
    scale_shape_manual(values = c("pp" = 15, 
                                  "rf" = 16, 
                                  "wf" = 17)) +
    theme_bw() +
...
lapply(sra.2001.ls, function(df) pro.plot(df))
```
>**Fig. `r {fig.n <- 1}`. Profiles of $\Delta$^14^C for 2001 samples**

>*Caption:* Within-site variability of $\Delta$^14^C for the 2001 samples.
...
sra.2001.sum <- bind_rows(sra.2001.sum.ls)
sra.2001.sum <- sra.2001.sum[order(sra.2001.sum$lyr_bot), ]
...
sra.2001.sum %>%
  ...
  scale_shape_manual(values = c("pp" = 15, 
                                "rf" = 16, 
                                "wf" = 17)) +
  facet_grid(rows = vars(PM), cols = vars(ECO)) +
  theme_bw() +
  theme(panel.grid.minor = element_blank())
```
...
ras18.lyr %>%
  ...
  theme(panel.grid.minor = element_blank())
```
>**Fig. `r {fig.n + 2}`. Mean profile $\Delta$^14^C for 2009 samples**
...
depth.spline <- function(x) {
  ...
  std <- seq(1, max(x$lyr_bot)+1, by = 1) # in cm
...
cwt <- lapply(seq_along(sra.2009.oc.sp), function(i) {
  d <- sort(sra.2001.sum.ls[[i]][["lyr_bot"]])
...
sra.01.09.ls <- lapply(seq_along(sra.2001.sum.ls), function(i) {
  sra.2001.sum.ls[[i]] <- sra.2001.sum.ls[[i]][order(sra.2001.sum.ls[[i]][["lyr_bot"]]), ]
  ...
  return(cbind(sra.2001.sum.ls[[i]], fm_01 = unlist(f)))
})
names(sra.01.09.ls) <- names(fm.wt)

nms <- c("PM", "ECO", "PMeco", "lyr_top", "lyr_bot", "fm", "fm_sd")
sra.01.09 <- sra.2001.sum[ , nms]
sra.01.09$fm <- unlist(lapply(sra.01.09.ls, function(df) df$fm_01))
sra.01.09$fm_sd <- NA 
sra.01.09$Year <- as.character(2009)
sra.01.09 <- rbind(cbind(sra.2001.sum[ , nms], Year = as.character(2001)), sra.01.09)
  
# plot 2001 and 2009 fm data
calc_fm <- function(d14c, obs_date_y) {
        lambda <- 0.00012097
        fm <- (((d14c/1000) + 1)/exp(lambda * (1950 - obs_date_y)))
        return(fm)
    }
atm.2001 <- calc_fm(graven[graven$Date == 2001.5, "NHc14"], 2001)
atm.2009 <- calc_fm(graven[graven$Date == 2009.5, "NHc14"], 2009)
```
...
sra.01.09 %>%
  mutate(PMeco_year = paste0(PMeco, Year),
         ...
         fm_u = fm + fm_sd,
         fm_l = fm - fm_sd) %>%
  ggplot(., aes(fm, lyr_bot, color = PM, shape = ecoYear, group = PMeco_year)) +
  geom_vline(xintercept = atm.2001) +
  geom_vline(xintercept = atm.2009, linetype = "dashed") +  
  geom_point(size = 3) +
  ...
  scale_color_manual(values = c("AN" = "blue", 
                                "BS" = "red", 
                                "GR" = "darkgray")) +
  scale_shape_manual(values = c("pp2001" = 15, 
                                "rf2001" = 16, 
                                "wf2001" = 17,
                                "pp2009" = 0, 
                                "rf2009" = 1, 
  ...
  theme(panel.grid.minor = element_blank())
```
>**Fig. `r {fig.n + 3}`. Mean profile $\Delta$^14^C for all sites in 2001 and 2009**
