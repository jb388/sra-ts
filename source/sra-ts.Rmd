...
sra.2019 <- list(GRrf = GRrf, GRwf = GRwf, GRpp = GRpp,
                 ANrf = ANrf, ANwf = ANwf, ANpp = ANpp,
                 BSrf = BSrf, BSwf = BSwf, BSpp = BSpp)
...
sra.2001 <- vector(mode = "list", length = length(sra.2019))
names(sra.2001) <- names(sra.2019)
...
soc.2001.ls <- lapply(split(soc.2001, soc.2001$PMeco), function(df) {
  df <- type.convert(df[complete.cases(df), c("ID", "C.", "mass_kgm2", "bd.g.cm3", "PMeco", "pro_rep", "lyr_top", "lyr_bot")])
...
# summarize SOC
soc.2001.sum <- data.frame(bind_rows(lapply(soc.2001.ls.inc, function(df) {
  df %>%
    mutate(ID2 = paste0(PMeco, "_", lyr_top, "-", lyr_bot)) %>%
    group_by(ID2, PMeco, lyr_top, lyr_bot) %>%
    dplyr::summarize(c_pct_avg = mean(C.))
})))
if (length(which(soc.2001.sum$ID2 == "BSrf_0-3" | soc.2001.sum$ID2 == "BSrf_3-8")) == 2) {
  soc.2001.sum2 <- soc.2001.sum[-which(soc.2001.sum$ID2 == "BSrf_0-3" | soc.2001.sum$ID2 == "BSrf_3-8"), ]
  soc.2001.sum2 <- rbind(
    soc.2001.sum2,
...
sra.2019.cn.sum <- data.frame(
  ...
  dplyr::summarize(across(c(C, N), .fns = mean))) %>%
  rename(c_pct_avg = C,
         n_pct_avg = N)
...
sra.2001$lyr_top <- as.numeric(sapply(strsplit(sapply(strsplit(sra.2001$ID, "-"), "[", 1), "_"), "[", 3))
sra.2001$lyr_bot <- as.numeric(sapply(strsplit(sapply(strsplit(sra.2001$ID, "-"), "[", 2), "_"), "[", 1))
...
# save inc list
save(sra.2019.inc.ls, file = "sra.2019.inc.ls.RData")
...
# add C content
ts[which(ts$year == 2001), "gC_gS"] <- as.numeric(soc.2001.sum2[match(ts[which(ts$year == 2001), "ID2"], soc.2001.sum2$ID2), "c_pct_avg"]) * 10^-2
...
depth.spline <- function(x, var_cmtv = "lyr_soc") {
  sp <- spline(x, method = "hyman") # fit monotonic cubic spline
  sp.ss <- smooth.spline(sp) # convert to class "spline" with smooth.spline fxn
  std <- seq(0, max(x$lyr_bot)) # in cm (linear beyond last measured depth)
  sp <- predict(sp.ss, std) 
  df <- data.frame(sp)
  colnames(df) <- c("lyr_bot", var_cmtv) # where var_cmtv = cumulative variable in spline output
  for(i in seq_along(df$lyr_bot)) {
    if(i == 1) {
      df[[var_cmtv]][i] <- df[[var_cmtv]][i]
    } else {
      df[[var_cmtv]][i] <- df[[var_cmtv]][i + 1] - df[[var_cmtv]][i]
    }
  }
  df <- df[-1,]
  return(df[-length(df[[var_cmtv]]), ])
}
...
# spline '01 fluxes
ts.rep.avg.sp <- lapply(split(ts.rep.avg, ts.rep.avg$year), function(y) {
  lapply(split(y, y$PMeco_year_rep), function(x) {
      x <- data.frame(x)
      if (x$year[1] == "2001") {
        # run mpspline
        depths(x) <- PMeco_year_rep ~ lyr_top + lyr_bot
        mps <- mpspline(x, var.name = "mgCO2_gC_d_avg", d = c(0, 10, 20, 30))
        mps.std <- mps$var.std
        data.frame(lyr_bot = c(10, 20, 30), flx = t(mps$var.std[1:3]), row.names = NULL)
      } else {
        data.frame(lyr_bot = x$lyr_bot, flx = x$mgCO2_gC_d_avg)
      }
  })
})
...
sra.2001.inc.ls2 <- sra.2001.inc.ls
# combine a and b reps for ANwf (only have data for one rep per increment)
sra.2001.inc.ls2$ANwf$rep <- "a" 
# run mpspline on fm data and return 10 cm increments for 0-30 cm
sra.2001.inc.sp <- bind_rows(lapply(sra.2001.inc.ls2, function(df) {
  df$PMeco_year_rep <- paste(df$PMeco, df$Year, df$rep, sep = "_")
  bind_rows(lapply(split(df, df$PMeco_year_rep), function(x) {
    depths(x) <- PMeco_year_rep ~ lyr_top + lyr_bot
    sp <- mpspline(x, "fm", d = c(0, 10, 20, 30))
    data.frame(lyr_bot = c(10, 20, 30),
               PMeco_year_rep = sp$idcol,
               fm = t(sp$var.std[1:3]))
  }))
}))
sra.2001.inc.sp.ls <- split(sra.2001.inc.sp, sra.2001.inc.sp$PMeco_year_rep)

# combine 14c and flux data
# 1. strip out missing data
ts.rep.avg.sp$`2001` <- ts.rep.avg.sp$`2001`[-which(names(ts.rep.avg.sp$`2001`) == "ANrf_2001_a" |
                                                  names(ts.rep.avg.sp$`2001`) == "ANwf_2001_b")]
# combine '01 data
flx.14c.rep.01.sp <- bind_rows(
  lapply(
    mapply(
      merge,
      ts.rep.avg.01.sp,
    ...
    function(x) {
      ...
      x$flx.wt <- x$flx / sum(x$flx)
      x$fm.wtd <- x$fm * x$flx.wt
      data.frame(PMeco_year_rep = unique(x$PMeco_year_rep), 
                 lyr_bot = max(x$lyr_bot), 
                 fm.wtd = sum(x$fm.wtd))
    })) %>%
  mutate(PMeco = substr(PMeco_year_rep, 1, 4),
         year = substr(PMeco_year_rep, 6, 9),
         d14c.wtd = convert_fm_d14c(fm = fm.wtd, obs_date_y = 2001, verbose = FALSE)) %>%
  rename(d14c_inc = d14c.wtd,
         fm_inc = fm.wtd)
  # group_by(PMeco, lyr_bot) %>%
  # summarize(across(c(fm.wtd, d14c.wtd), .fns = list(mean = mean, sd = sd)))
save(flx.14c.rep.01.sp, file = "flx.14c.rep.01.sp.RData")
```

```{r spline-cwt-14c}
# fit weights to blkC and inc data
...
blkC.rep.14c.sp.wtd.ls <- lapply(sra.2001.ls, function(df) 
  lapply(split(df, df$pro_rep), wtd.sp.fx, target_var = "fm", weight_var = "lyr_soc_kgm2"))
...
# split by rep and fit splines to fluxes and fm
flx.rep.14c.sp.wtd.19.ls <- lapply(flx.rep.14c.19.ls, function(df) 
  lapply(split(df, df$rep), wtd.sp.fx, target_var = "fm", weight_var = "mgCO2_gC_d_avg"))

# sum for 0-30 cm
flx.rep.wtd.30.14c.19.ls <- lapply(flx.rep.14c.sp.wtd.19.ls, function(x) {
  sum.30.fx(x, "fm_wtd") %>%
    mutate(d14c_wtd = convert_fm_d14c(fm = fm_wtd, obs_date_y = 2019, verbose = FALSE))
})
...
## '19 bulk data
# split by rep and fit splines to SOC and fm
blkC.rep.14c.sp.wtd.19.ls <- lapply(sra.2019.ls, function(df) 
  lapply(split(df, df$pro_rep), wtd.sp.fx, target_var = "fm", weight_var = "lyr_soc"))

# sum for 0-30 cm
blkC.rep.wtd.30.14c.19.ls <- lapply(blkC.rep.14c.sp.wtd.19.ls, function(x) {
  sum.30.fx(x, "fm_wtd") %>%
    mutate(d14c_wtd = convert_fm_d14c(fm = fm_wtd, obs_date_y = 2019, verbose = FALSE))
})
blkC.rep.wtd.30.14c.19.df <- bind_rows(blkC.rep.wtd.30.14c.19.ls, .id = "PMeco") %>%
  mutate(year = 2019)

# combine 0-30 cm data
inc.blk.rep.30.wtd.df <- merge(
  rbind(blkC.rep.wtd.30.14c.df, blkC.rep.wtd.30.14c.19.df),
  rbind(flx.rep.wtd.30.14c.df, flx.rep.wtd.30.14c.19.df),
  by = c("PMeco", "year", "lyr_bot"),
  suffixes = c("_blk", "_inc")
)
save(inc.blk.rep.30.wtd.df, file = "inc.blk.rep.30.wtd.df.RData")
```
...
# plot function
C14.2p.plot.fx <- function(plot.df, con.df, mod, pool_fltr = "bulk & resp", PMeco_depth) {
  
  if (pool_fltr == "bulk & resp") {
    plot.df <- plot.df %>%
      filter(pool == "bulk C" | pool == "respiration" | pool == "atm")
  }
  
  p <- ggplot(plot.df, aes(years, d14C, color = pool)) +
  ...
  if (pool_fltr != "bulk & resp") {
    p + 
      geom_path(aes(linetype = pool)) +
      scale_linetype_manual(name = "Pool",
                            values = c("atm" = 1,
                                       "bulk C" = 1,
                                       "fast" = 2,
                                       "slow" = 2,
