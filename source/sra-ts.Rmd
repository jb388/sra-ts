        ...
        df %>%
          ...
          scale_linetype_manual(name = "Climate",
                                ...
                                labels = c("rf" = "cold",
                                        "wf" = "cool",
                                        "pp" = "warm")) +
          ylab(expression('Respiration Rate (mgCO'[2]*'-C gC'^-1*')')) +
          xlab("Time (days)") +
...
summary(lm(d14c_mean.inc ~ d14c_mean.bulk * PM, sra.all.sum.df))

# # Deming regression (accounts for error in x and y terms)
# deming(d14c_mean.inc ~ d14c_mean.bulk,
#        data = sra.all.sum.df[sra.all.sum.df$PM == "AN", ], 
#        xstd = d14c_sd.inc, ystd = d14c_sd.bulk)
# deming(d14c_mean.inc ~ d14c_mean.bulk,
#        data = sra.all.sum.df[sra.all.sum.df$PM == "BS", ], 
#        xstd = d14c_sd.inc, ystd = d14c_sd.bulk)
# deming(d14c_mean.inc ~ d14c_mean.bulk,
#        data = sra.all.sum.df[sra.all.sum.df$PM == "GR", ], 
#        xstd = d14c_sd.inc, ystd = d14c_sd.bulk)
...
sra.01.09.19 %>%
  ...
  theme(panel.grid.minor = element_blank())
```
>**Fig. `r {fig.n}`. Time series of bulk soil $\Delta$^14^C by depth (2001, 2009, 2019)**
...
sra.19.01.inc %>%
  ...
  geom_path(data = atm.14c) +
  geom_point(aes(color = pm, shape = eco), size = 3) +
  geom_point(data = sra.2019.inc.L.df, aes(color = pm), shape = 8, size = 3, show.legend = FALSE) +
  geom_path(aes(color = pm, group = PMeco_depth, linetype = depth), alpha = 0.3) +
  geom_errorbar(
    aes(ymin = d14c_min, 
        ymax = d14c_max,
...
par.fx <- function(pars, In, lag = 5, out = "plot.df", verbose = TRUE, mod, pass = FALSE) {
...
mod.fits.fx <- function(mod, pars, In, sub, upper, lower, cost) {
  ...
  lapply(seq_along(pars[sub]), function(i) {
    ...
    if (cost == "14C + cStock") {
      mod.Cost <- function(pars) {
        modelOutput <- modFun_2p(pars, In, mod = mod)
...
# GRwf 0-10
Twop.fit.plot.fx(Twops.10br.fits[which(names(Twops.10br.fits) == "GRwf_0-10")], 
                 "2ps 0-10cm w/ resp", 
                 Twops.10b.fits[which(names(Twops.10b.fits) == "GRwf_0-10")],
                 "2ps 0-10cm w/o resp")
# BSrf 0-10
...
# compare resp + bulk fits w/ and w/o stocks
Twop.fit.plot.fx(Twops.10br.fits,
                 "2ps 0-10cm w/ resp + bulk 14C",
                 Twops.10b.fits, 
                 "2ps 0-10cm w/ bulk 14C only")
```
