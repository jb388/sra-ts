...
sra.09.sum %>%
  ...
  theme(panel.grid.minor = element_blank())
```
>**Fig. `r {fig.n + 1}`. Profile $\Delta$^14^C for 2009 samples**

>*Caption:* Profile $\Delta$^14^C by depth for each site in 2009. Error bars not shown as only a single replicate profile was analyzed per site.
...
>*Caption:* Mean $\Delta$^14^C by depth for each site in 2019. Error bars show ±1 standard deviation.
...
sra.all %>%
  ...
  theme(panel.grid.minor = element_blank())
```
>**Fig. `r {fig.n + 3}`. Mean profile $\Delta$^14^C for 2001 and 2019 samples**

>*Caption:* Mean $\Delta$^14^C by depth for each site in 2001 and 2019. Error bars show ±1 standard deviation.

## Incubation $\Delta$^14^C-CO~2~ for 2019 bulk soils

```{r plot-inc-d14c}
sra.2019.inc.df_1 <- bind_rows(sra.2019.inc.ls_1)
# remove GRrf 10-20 point
sra.2019.inc.df_1[which(sra.2019.inc.df_1$PMeco == "GRrf" & sra.2019.inc.df_1$lyr_top == "10"), "d14c"] <- NA
...
sra.2019.inc.df_1 %>%
  ...
  theme(panel.grid.minor = element_blank())
```
>**Fig. `r {fig.n + 4}`. Rep 1 $\Delta$^14^C-CO~2~ of 2019 bulk soil incubations**

>*Caption:* $\Delta$^14^CO~2~ by depth for each site in 2019. (Still waiting on rep 2 data)
...
sra.19.inc.blk %>%
  ...
  theme(panel.grid.minor = element_blank())
```
>**Fig. `r {fig.n + 5}`. $\Delta$^14^C of 2019 bulk soil incubations and corresponding bulk soil**
...
#   theme(panel.grid.minor = element_blank())
```
>**Fig. `r {fig.n + 6}`. Regression of 2019 bulk soil incubations and corresponding bulk soil $\Delta$^14^C**
...
Non-trivial to match 2009 bulk density to 2019 depth increments. How to do this? Need a "closest match" function. Or predict from C content using regression? Regression approach fits poorly: best R^2^ is `r {summary(lm(lyr_bd_samp ~ lyr_c_org * lyr_bot + PM + ECO, ras18.lyr))$r.squared}`. Actually, best approach is probably to use a spline function to predict bulk density for new intervals.
...
sra.01.09.19 %>%
  ...
  theme(panel.grid.minor = element_blank())
```
>**Fig. `r {fig.n + 7}`. Mean profile fraction modern for all sites in 2001 and 2009**
