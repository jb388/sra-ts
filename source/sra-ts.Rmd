...
# initial pars
In <- ifelse(lyr_bot == 10, In.10, ifelse(lyr_bot == 20, In.20, In.30))
...
# initial pars
In <- ifelse(lyr_bot == 10, In.10, ifelse(lyr_bot == 20, In.20, In.30))
gam <- .78 # partitioning coef
ks <- c(.1, .0022) # fast, slow
...
# initial pars
In <- ifelse(lyr_bot == 10, In.10, ifelse(lyr_bot == 20, In.20, In.30))
gam <- .85 # partitioning coef
ks <- c(.08, .0015) # fast, slow
...
# initial parameters
In <- ifelse(lyr_bot == 10, In.10, ifelse(lyr_bot == 20, In.20, In.30))
gam <- .85 # initial value
ks <- c(.055, .0013) # fast, slow
...
# initial pars
In <- ifelse(lyr_bot == 10, In.10, ifelse(lyr_bot == 20, In.20, In.30))
gam <- .75 # partitioning coef
ks <- c(.07, .0042) # fast, slow
...
# initial pars
In <- In.10
gam <- .65 # partitioning coef
ks <- c(.09, .0042) # fast, slow
...
# initial pars
In <- ifelse(lyr_bot == 10, In.10, ifelse(lyr_bot == 20, In.20, In.30))
gam <- .5 # partitioning coef
ks <- c(.3, .0028) # fast, slow
...
mod.fits <- lapply(seq_along(pars.i), function(i) {
  start <- Sys.time()
...
# save output
names(mod.fits) <- names(pars.i)
save(mod.fits, file = paste0("../data/derived/modFit_pars/modFit_pars_", Sys.Date(), ".Rdata"))
```

```{r report-par-fit}
# load fits
load(file = "../data/derived/modFit_pars/modFit_pars_2020-11-09.RData")
...
names(pars.fit) <- names(pars.i)

# summary of fits
pars.fit.sum <- lapply(mod.fits, function(x) summary(x))
names(pars.fit.sum) <- names(pars.fit)

# bind fitted parameters into data frame for plotting/summarizing
pars.fit.df <- bind_rows(pars.fit)
pars.fit.df$PMeco_depth <- rep(names(pars.i), each = 2)
...
pars.fit.df$depth <- substr(pars.fit.df$PMeco_depth, 
                            start = 6, 
                            stop = length(pars.fit.df$PMeco_depth))

# summarize by PM, depth
pars.fit.df.PM <- suppressMessages(pars.fit.df %>%
  filter(est == "fit") %>%
...
pars.fit.df.eco <- suppressMessages(pars.fit.df %>%
  filter(est == "fit") %>%
...
soc.fx <- function(ks, gam, In) {
  colSums(-1 * solve(-1 * diag(ks)) %*% c(In * gam, In * (1 - gam)))
}

in.fit.fx <- function(ks, gam, initialIn, SOC) {
  # sequence of possible input values
  if  (SOC < soc.fx(ks, gam, initialIn)) {
    ins <- seq(.01, 
               initialIn, 
               .01)
    } else {
...
# run function
in.fit <- lapply(seq_along(ks.ls), function(i) {
  PMeco_depth <- names(ks.ls)[i]
  SOC <- csoc.19.0_30[[PMeco_depth]][ ,"lyr_soc"]
  if (grepl("0-10", PMeco_depth)) {
    return(in.fit.fx(ks.ls[[i]], gam.ls[[i]], In.10, SOC))
  } else if (grepl("10-20", PMeco_depth)) {
    return(in.fit.fx(ks.ls[[i]], gam.ls[[i]], In.20, SOC))
  } else {
    return(in.fit.fx(ks.ls[[i]], gam.ls[[i]], In.30, SOC))
  }
})
...
mod.socs.ls <- lapply(seq_along(ks.ls), function(i) {
  A <- -1 * diag(ks.ls[[i]])
  return(colSums(-1 * solve(A) %*% c(in.fit[[i]] * gam.ls[[i]], in.fit[[i]] * (1 - gam.ls[[i]]))))
})
...
Twopp.fits <- lapply(seq_along(ks.ls), function(i) {
  par.fx(in.fit[[i]], gam.ls[[i]], ks.ls[[i]], verbose = FALSE)
})
...
Twopp.fits.plots <- lapply(seq_along(Twopp.fits), function(i) {
  ...
  plot.df <- rbind(Twopp.fits[[i]], 
                   par.fx(ifelse(lyr_bot == 10, In.10, ifelse(lyr_bot == 20, In.20, In.30)),
                          pars.i[[i]][3], 
                          pars.i[[i]][1:2], 
                          verbose = FALSE))
...
plot(bayes_fit_2pp[[1]])

## look at model performance
pars.bayes.df <- bind_rows(lapply(seq_along(bayes_fit_2pp), function(i) {
  df <- data.frame(k1 = unique(bayes_fit_2pp[[i]][["pars"]][, 1]),
                   k2 = unique(bayes_fit_2pp[[i]][["pars"]][, 2]),
                   gam = unique(bayes_fit_2pp[[i]][["pars"]][, 3]))
  df <- cbind(PMeco_depth = rep(names(pars.fit)[i]), 
              df)
}))
...
# best par set
bestPars.bayes.ls <- lapply(bayes_fit_2pp, function(x) {
